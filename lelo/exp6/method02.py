# Method 2: Using Positive and Negative Word Count – With Semi Normalization for Calculating Sentiment Score
import re
...
def sentiment_score_semi(tokens: list, positive_words: set, negative_words: set) -> float:
    pos = sum(1 for t in tokens if t in positive_words)
    neg = sum(1 for t in tokens if t in negative_words)
    denominator = pos + neg
    if denominator == 0:
        return 0.0
    return round((pos - neg) / denominator, 6)
...
df["score"] = df["tokens"].apply(lambda t: sentiment_score_semi(t, POSITIVE_WORDS, NEGATIVE_WORDS))
