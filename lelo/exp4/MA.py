...
# Your current MA script only needs one column from a CSV: MaxTemp.
...
plt.plot(range(len(train), len(data)), forecast, label='MA Forecast', marker='x', linestyle='--', color='red')
plt.xlabel('Day')
plt.ylabel('Max Temperature (°C)')
plt.legend()
plt.title('MA Model - MaxTemp Forecast')
plt.tight_layout()
plt.show()
